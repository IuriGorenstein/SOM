...
def EvolPlot_pend(t):
    Imprime_evolucao_pend(som,cluster,Prct,PATTERN,t)
    
    
def Imprime_evolucao_pend(som,clusters,porcentagem,PATTERNsst,t):
    ...
    a= plot_hex_maps_pend(np.flip(clusters.reshape(msz[0], msz[1])[::], axis=0),clusters,porcentagem[:,t],PATTERNsst, colorbar=False,fig=fig)
    
    
def plot_hex_maps_pend(d_matrix,clusters,porcentagem,PATTERNsst, titles=[], shape=[1, 2], comp_width=5, hex_shrink=1.0, fig=None,
                 colorbar=True):
    """
    Plot hexagon map where each neuron is represented by a hexagon. The hexagon
    color is given by the distance between the neurons (D-Matrix)
    ...
    for comp, title in zip(range(d_matrix.shape[2]), titles):
        ...
        # Filtrando Padrões que tem correlção
        filtrado = PATTERN[masc]       
        for i in range(filtrado.shape[1]):
            filtrado[:,i]=filtrado[:,i]*porcentagem[masc]
              
        ay = fig.add_subplot(1, 6, comp + 2, aspect='equal',
                              projection='polar')
        #plt.rcParams["figure.figsize"] = [7.00, 3.50]
        #plt.rcParams["figure.autolayout"] = True
        ay.set_theta_direction(-1)
        ay.set_theta_offset(np.pi / 2.0)
        theta = np.linspace(0, 2 * np.pi, 37)
        ay.plot(np.sin(np.nanmean(filtrado,axis=0)),range(64))
